...
function logger(func, logFunc) {
  return (...x) => {
    const param = JSON.stringify(x);
    logFunc(`${func.name}(${param.slice(1, param.length - 1)}) starts`);
    const res = func(...x);
    logFunc(`${func.name}(${param.slice(1, param.length - 1)}) ends`);
    return res;
  };
}
